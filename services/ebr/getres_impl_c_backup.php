...
function getres_impl_c($env=null)
{
  ...
  $pret = new stdClass();
  $pret->status = 1;
  $pret->msg = "Something is not loaded properly.";
  $pret->res = "";

  if (!isset($env['CAN_VIEW_RESULT'])) goto the_end;


  #require("common_defs.php");
  $exams = array_keys($defs['exam_map']); # defined in common_defs.php
  ...
  @$requested_result_type = preg_replace("/[^0-9-]+/", "", $_REQUEST['result_type']);
  if ($requested_result_type == "") $requested_result_type = "1"; # so that result API works as expected

  $requested_cname = false;
  if (isset($_REQUEST['cname']) && $_REQUEST['cname'] == '1') { # Center Name display (requested by Murad, JB)
    $requested_cname = true;
  }

  if (!in_array($requested_result_type, $defs['allowed_result_type'])) {
    #file_put_contents(sys_get_temp_dir() . "/app_stud_attempt.txt", date("r") . ": " . $_SERVER['REMOTE_ADDR'] . ":\n" . print_r($_REQUEST, true) . "\n\n", FILE_APPEND);
    $pret->status = 1;
    $pret->msg = "This category of result/service is not available at this moment!";
    goto the_end;
  }

  # check existence of Mandatory Parameters (variables)


  # This will dramatically reduce server load until the moment comes
  $npubfile = "${requested_exam}.${requested_year}.notpublished"; # The file will have the notice text as well
  if (!isset($env['TRIAL_RESULT']) && file_exists($npubfile)) {
    # check if there is any notice (valid) to show, otherwise load file content
    $notice_file = "notice.php";
    include($notice_file);
    $pret->status = 1;
    $pret->msg = "Result Not Published Yet! ";
    if (isset($DISPLAY_NOTICE) && $DISPLAY_NOTICE == true) {
      $pret->msg .= $notice_msg; # notice_msg is set in notice.php
    } else {
      $pret->msg .= file_get_contents($npubfile);
    }
    goto the_end;
  ...
  if (array_key_exists($requested_exam, $defs['exam_translate'])) {
  ...
  if (!in_array($requested_exam, $exams)) {
    $pret->status = 1;
    $pret->msg = "$requested_exam not allowed";
    goto the_end;
  ...
  if ($requested_exam == "dibs") {
    ...
    $requested_exam = "hsc"; # table name: hsc_dibs_yyyy
  }
  $requested_board_upper = strtoupper($requested_board);


  # Institution Result Triggered here (if chosen)
  if (in_array($requested_result_type, $defs['allowed_result_type_group'])) {
    require("result_pdf" . $env['SUFFIX'] . ".php");
    goto the_end;
    #die();
  }
  ...
  /*
  if (isset($allowed_result_map[$requested_result_type])) {
    require($allowed_result_map[$requested_result_type]);
    die();
  }
   */
  ...
  #$sub_tbl = "sub_tbl.php"; # using cdn for load balancing
  #$sub_tbl = "http://cdn.eboardresults.com/eb/app/stud/sub_tbl.php"; # During result it (when resolved to ebaordresults.com server) gets lots of pressure already
  $sub_tbl = "sub_tbl.php";
  ...
  */
  ...
  #  }
	...
	*/
	...
	/*
	if ($codes_str) {
	  $pret->sub = "$sub_tbl?file=$subjects_map_file&code=$codes_str"; # This will be removed in near future due to above sub_details
	}
	 */
      }
    }
  ...
  $res = json_encode($pret);
  if ($pret->status == 0 && isset($env['CACHE_ID']) && isset($env['CACHE_KEY']) && isset($env['CACHE_KEY1'])) {
    $curtime = time();
    $server->cache[$env['CACHE_ID']]->set($env['CACHE_KEY'], [ $env['CACHE_KEY1'] => $res, '__ts' => $curtime ]);
    if (in_array($requested_result_type, array(2,4,5,7))) {
      $server->cache['gsession']->set($env['CACHE_KEY'], [ 'res' => serialize($_SESSION), '__ts' => $curtime ]);
    }
  }
  return $res;
}
